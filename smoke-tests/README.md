...
| [75-extension.sh](75-extension.sh) | `--extension`: drive a normal browser through the **pwhs bridge** extension. Launches a host Chromium with the unpacked extension (via [_host-browser.ts](_host-browser.ts)), pairs it hands-free with `pwhs pair` (the extension reads the token from the local pairing page), then proves every verb through the bridge: profiles catalog, nav, ai refs, screenshots, activity, new tabs + `pages`/`switch`/`latest`, raw CDP incl. `Browser.*` translation, tracing, clock, downloads, `restart tab=N`, and that `stop` leaves the browser open. |
